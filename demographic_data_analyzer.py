...
def calculate_demographic_data(print_data=True):
    ...
    counts = np.zeros(len(races), np.int32)
    for index, race in enumerate(races):
        counts[index] = df[ df['race'] == race ]['race'].count()

    race_count = pd.Series(counts, index=[races], name="Race_Count")
    ...
    percentage_bachelors = (bachelors/df.shape[0]) * 100
    ...
    rich_percentage = (df[ (df['hours-per-week'] == df['hours-per-week'].min()) & (df['salary'] == '>50K') ].count() / num_min_workers) * 100
